...
if __name__ == '__main__':
    ...
    print("Load datasets")
    train_dataset, valid_test_dataset = prepare_datasets(source, target, ratio)
    ...
    data_log = "source (%s) train : %d, valid %d| target (%s) train: %d, test %d" % (
        source, train_source_size, valid_size, target,
        train_target_size, test_size)
    print(data_log)
    valid_sampler = data.sampler.SubsetRandomSampler(
        [i for i in range(0, valid_size)])
    test_sampler = data.sampler.SubsetRandomSampler(
        [i for i in range(
            valid_size, valid_test_dataset.cummulative_sizes[1])])
    source_sampler = data.sampler.SubsetRandomSampler(
        [i for i in range(0, train_source_size)])
    valid_loader = data.DataLoader(
        valid_test_dataset, batch_size=optim['batch_size'],
    ...
    reg = DANReguralizer(
        learner=learner, discriminator_config=discriminator_config)
    ...
    for batch_idx in range(optim['num_batch']):
        ...
        external_result = da_check_invariance(
            learner.E, train_dataset, 1000, lr=0.001,
            hiddens=[E.output_shape()[1]], verbose=0)
        d_log = "valid domain loss: %.4f || external domain acc: %.4f " % (
            valid_result['d-loss'], external_result['valid-domain-accuracy'])
        # HDivergence
        ...
        base_log = "%s [%06d, %d s (%d s, %d s)] || valid acc: %.3f, valid loss: %.4f|| test acc: %.3f, test loss: %.4f " % (
            "DAN", batch_idx+1, int(elapsed_time2), int(elapse_train_time),
            int(elapsed_time1), valid_result['y-accuracy'],
            valid_result['y-loss'], test_result['y-accuracy'],
            test_result['y-loss'])
        print(base_log + ' || ' + d_log)
        if tbx:
            writer.add_scalar(
                'c_loss/valid/%s' % source,
                valid_result['y-loss'], batch_idx)
            writer.add_scalar(
                'c_acc/valid/%s' % source,
                valid_result['y-accuracy'], batch_idx)
            writer.add_scalar(
                'c_loss/test/%s' % target,
                test_result['y-loss'], batch_idx)
            writer.add_scalar(
                'c_acc/test/%s' % target,
                test_result['y-accuracy'], batch_idx)
            writer.add_scalar(
                'check_invariance/acc',
                external_result['valid-domain-accuracy'], batch_idx)
